...
This analysis follows the [CNL_Prep_Stats_PHS](https://github.com/shantel-martinez/CNL_GWAS_GP2020/blob/master/Data%20Analysis/CNL_Prep_Stats_PHS.md) analysis in the [CNL_GWAS_GP2020](https://github.com/shantel-martinez/CNL_GWAS_GP2020) project. The environment should contain only the following dataframes:
...
If these dataframes are **not** in the current environment, open the [CNL_Prep_Stats_PHS.Rmd](https://github.com/shantel-martinez/CNL_GWAS_GP2020/blob/master/Data%20Analysis/CNL_Prep_Stats_PHS.Rmd) file and run all coding chunks. *Be sure to have all the corresponding input files needed to run*   
OR  
Download the [CNL_Prep_Stats_PHS.Rdata](https://github.com/shantel-martinez/CNL_GWAS_GP2020/blob/master/Data%20Analysis/CNL_Prep_Stats_PHS.RData) files from github, save in the working directory this CNL_GWAS_PHS.Rmd is in. 
...
> `myGM` The first column is the SNP id, the second column is the chromosome, and the third column is the base pair position
> `myGD` and `myGM` must be in the same order 
...
The red dataset with 4PC as covariates is overfitted, notice the values drop below the baseline (red). There were also no significant QTN reported in the red_4PC GWA analysis (potentially due to an overfit model).  
...
White <- read.csv("./20200409/YrHarv_4PC/FarmCPU.BLUP_White.GWAS.Results.csv", head = TRUE)
White$KC <- "White"
Red <- read.csv("./20200409/YrHarv_4PC/FarmCPU.BLUP_Red.GWAS.Results.csv", head = TRUE)
Red$KC <- "Red"

PHS4 <- rbind( White, Red)
PHS4$chrom <- as.character(PHS4$chrom)
PHS4$chrom[which(PHS4$chrom == "1")] <- "1A"
PHS4$chrom[which(PHS4$chrom == "2")] <- "2A"
PHS4$chrom[which(PHS4$chrom == "3")] <- "3A"
PHS4$chrom[which(PHS4$chrom == "4")] <- "4A"
PHS4$chrom[which(PHS4$chrom == "5")] <- "5A"
PHS4$chrom[which(PHS4$chrom == "6")] <- "6A"
PHS4$chrom[which(PHS4$chrom == "7")] <- "7A"
PHS4$chrom[which(PHS4$chrom == "8")] <- "1B"
PHS4$chrom[which(PHS4$chrom == "9")] <- "2B"
PHS4$chrom[which(PHS4$chrom == "10")] <- "3B"
PHS4$chrom[which(PHS4$chrom == "11")] <- "4B"
PHS4$chrom[which(PHS4$chrom == "12")] <- "5B"
PHS4$chrom[which(PHS4$chrom == "13")] <- "6B"
PHS4$chrom[which(PHS4$chrom == "14")] <- "7B"
PHS4$chrom[which(PHS4$chrom == "15")] <- "1D"
PHS4$chrom[which(PHS4$chrom == "16")] <- "2D"
PHS4$chrom[which(PHS4$chrom == "17")] <- "3D"
PHS4$chrom[which(PHS4$chrom == "18")] <- "4D"
PHS4$chrom[which(PHS4$chrom == "19")] <- "5D"
PHS4$chrom[which(PHS4$chrom == "20")] <- "6D"
PHS4$chrom[which(PHS4$chrom == "21")] <- "7D"
target <- c("1A", "2A", "3A","4A","5A","6A", "7A", 
            "1B", "2B", "3B","4B","5B","6B", "7B", 
            "1D", "2D", "3D","4D","5D","6D", "7D")
require(gdata)
PHS4$chrom<- reorder.factor(PHS4$chrom, new.order=target)
PHS4$log10p <- -(log10(PHS4$P.value))
p.threshold=-(log10(0.05/nrow(myGM)))

ggplot(PHS4) +
  geom_point(aes( x = PHS4$pos, y =PHS4$log10p, colour = ifelse( PHS4$log10p < p.threshold,'grey',PHS4$KC)))+
  geom_hline(yintercept = p.threshold, colour = "grey38", linetype="dashed")+
  facet_wrap(. ~ chrom, ncol =7) +
  scale_color_manual(values=c("grey","#FFD39B","#8B0000"))+theme_bw() +
  ylab("-log(p-value)") + xlab("Chromosome Physical Position (RefSeq v1.0)")+
  theme(legend.position="none",axis.text.x = element_text(colour = "white"))
 # ggsave("./20200409/ManhPlot_Sprout_rrBLUP_20190503.png", width = 16, height = 10, units = "in")
```
