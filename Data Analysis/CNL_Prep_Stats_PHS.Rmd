...
a <- ggplot(PHSWhite1, aes(x = PHSWhite1$Year, y = PHSWhite1$RawMean))+
  ...
  ylab("Naive Sprouting Mean") + xlab("Environment")+ 
  guides(fill=FALSE) + ylim(0, 9) +
  ggtitle("A)")+scale_fill_grey()+
  geom_text(data = dat_text,mapping = aes(x =0.5, y = 8.5, label = label),
...
b <- ggplot(PHSwGIDblup, aes(PHSwGIDblup$KC, PHSwGIDblup$blup...GID...)) +
  ...
  ylab("Sprout BLUP") + xlab("") + ylim(-3,3)+
  facet_grid(.~Env)+ggtitle("B)")+
  annotate("text", x =1, y = -2.5, label = paste("italic(H)[C]^2 == ",h2w),  parse = TRUE,size =6)
...
d <- ggplot(PHSrGIDblup, aes(PHSrGIDblup$KC, PHSrGIDblup$blup...GID...)) +
  ...
  ylab("Sprout BLUP") + xlab("") + ylim(-3,3)+
  facet_grid(.~Env)+ggtitle("D)")+
  annotate("text", x =1, y = -2.5, label = paste("italic(H)[C]^2 == ",h2r),  parse = TRUE,size =6)

library(gridExtra)
a <- a+ xlab("")
p <- grid.arrange(a, b, c,d, nrow = 2,  widths = c(5, 1),heights = c(14,14))
ggsave("../Data Analysis/CNL_Prep_Stats_PHS_files/SproutMeansGID_20200420.png", plot =p,width = 18, height = 10, units = "in")
